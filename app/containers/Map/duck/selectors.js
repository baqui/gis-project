export const getVoivodeships = state =>
  state.voivodeships.voivodeships.get('list').toArray();
...
export const isVoivodeshipsWeatherDataFetched = state =>
  state.voivodeships.voivodeships.get('weather_data_fetched');
...
export const getChosenVoivodeshipCoordinates = state => {
  const voivodeships = getVoivodeships(state);
  const cartodb_id = getCheckedVoivodeshipId(state);
  if (cartodb_id && voivodeships[cartodb_id]) {
    return voivodeships[cartodb_id].coordinates;
  }
...
export const getWeatherRange = (state, type) => {
  let temps = state.voivodeships.voivodeships
    .get('weather')
    .toArray()
    .filter(a => a)
    .map(weather => weather[type]);
...
export const getMapViewport = state => state.voivodeships.grid_temperature.viewport;

export const getMapZoom = state => state.voivodeships.grid_temperature.zoom;

export const getMapGrid = state => state.voivodeships.grid_temperature.grid.toJS();